...
export default function LocationBox({ data }: any) {
  if (!data) return null;

  const title = data?.sectionTitle;
  const subtitle = data?.sectionSubtitle;
  const email = data?.przyciski?.buttonEmail?.emailAdress;
  const phone = data?.przyciski?.phoneButton?.phoneNumber;
  const map = data?.przyciski?.mapa;
  ...
  return (
    <section
      id="kontakt"
      className="relative w-full py-32 md:py-48 bg-[#050505] text-[#FAFAF9] overflow-hidden"
    >
      ...
      <div className="max-w-[1600px] mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-16 md:gap-24 items-center relative z-10">
        ...
        <div className="col-span-1 md:col-span-5 space-y-12">
          ...
          <div className="space-y-8">
            ...
            {phone && (
              <a
                href={`tel:${phone}`}
                className="group flex items-start gap-6 border-l border-white/10 pl-6 hover:border-[#D4A373] transition-colors duration-300"
              >
        ...
        <div className="col-span-1 md:col-span-7 relative h-[500px] md:h-[600px] w-full">
          ...
          <div className="absolute inset-0 border border-white/5 m-4 md:m-8 pointer-events-none"></div>
            ...
            >
              <div className="bg-[#050505]/80 backdrop-blur-md border border-white/10 px-6 md:px-8 py-4 md:py-5 flex items-center gap-4 hover:bg-[#D4A373] hover:border-[#D4A373] hover:text-black transition-all duration-300">
                <div className="w-10 h-10 bg-white/10 flex items-center justify-center">
                  <FaMapMarkerAlt
                    className="text-[#D4A373] group-hover:text-black"
                    size={20}
                  />
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-neutral-300 group-hover:text-black/70 mb-1">
                    GPS
                  </p>
