...
function CabinCard({ cabin, idx }) {
  ...
  return (
    <Link
      href={cabin.slug}
      className={`group block rounded-[2.5rem] overflow-hidden border transition-all duration-700 ease-out min-h-[600px] relative ${cardBg} ${cardBorder} ${shadowClass} hover:-translate-y-1`}
    >
      <div className="flex flex-col lg:flex-row h-full">
        <div className="relative w-full lg:w-1/2 h-72 lg:h-auto min-h-[350px] overflow-hidden">
          <div
            className={`absolute top-6 left-6 z-20 inline-flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md shadow-sm border transition-all duration-300 ${statusBg}`}
          >
            <span
          ...
          <div className="absolute bottom-6 left-6 z-20 w-12 h-12 relative">
            <Image
              src={cabin.logo}
              alt={cabin.name}
              fill
        ...
        <div
          className={`w-full lg:w-1/2 p-8 md:p-12 flex flex-col justify-between ${cardBg}`}
        >
          <div className="flex flex-col gap-4 mb-2">
            <div className="flex items-start justify-between gap-4">
              ...
              <div>
                <h3
                  className={`text-3xl md:text-4xl font-serif font-medium leading-none mb-1 ${textColor}`}
                >
                  {cabin.name}
                </h3>
            ...
            <div className={`border-t ${dividerColor} pt-6`}>
              <div className="relative h-12 overflow-visible mb-6">
                <div
                ...
                >
                  <span className={`text-4xl font-serif ${textColor}`}>
                    {cabin.price}
                  </span>
                  <span
                    className={`text-sm font-sans font-normal ${subTextColor}`}
                  >
                    {cabin.period}
                  </span>
                </div>
                ...
                >
                  <span className="text-2xl text-stone-500 line-through decoration-red-500/50 decoration-2 mr-2">
                    {basePrice * packageDays} zł
                  </span>
                  <span className="text-4xl font-serif text-[#D4A373]">
                    {packagePrice} zł
                  </span>
                  <span
                    className={`text-sm font-sans font-normal text-[#D4A373]/80`}
                  >
                    / {packageDays} noce
                  </span>
                </div>
              ...
              <div className="relative bg-[#0A0A0A] rounded-xl p-1 flex mb-2">
                ...
                <button
                  ...
                  className={`flex-1 relative z-10 py-2.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 flex items-center justify-center gap-2
          ...
          <div className="flex-grow flex flex-col justify-between">
            <div
              className={`grid grid-cols-3 gap-4 mb-6 ${dividerColor} border-t pt-6`}
            >
              <div
                className={`text-center p-3 ${isDark ? "bg-[#111]" : "bg-stone-100"} rounded-xl ${isDark ? "border-[#222]" : "border-stone-300"} transition-colors group-hover:bg-[#D4A373]/20`}
              >
            ...
            <ul className="space-y-2 mb-6">
              {cabin.features.map((feature: any, fIdx: any) => (
                <li
                  key={fIdx}
                  className={`flex items-center gap-2 text-sm font-light ${isDark ? "text-stone-400" : "text-stone-600"}`}
                >
            ...
            <div
              className={`group/btn flex items-center justify-between ${dividerColor} border-t pt-6`}
            >
              <span
                className={`font-bold uppercase tracking-widest text-xs group-hover/btn:text-[#D4A373] transition-colors duration-300 ${textColor}`}
              >
                Zobacz szczegóły
              </span>
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${btnBg} ${btnHover}`}
              >
