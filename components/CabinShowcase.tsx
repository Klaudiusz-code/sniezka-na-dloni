...
interface Cabin {
  ...
  zdjecieGlowne?: { node: { sourceUrl: string } } | null;
  cena: string;
  okres: string;
  liczbaGosci: string;
  liczbaSypialni: string;
  metraz: string;
  domekWyrozniki?: { wyroznik: string }[] | null;
  krotkiOpisPodNaglowkiem: string;
  domekStruktura?: { tytul: string }[] | null;
  domekUdogodnienia?: { udogodnienie: string }[] | null;
  domekOpiniaTresc: string;
  galeriaZdjec?:
    | { galleryImage: { node: { sourceUrl: string } } | null }[]
    | null;
}
          ...
          <div className="flex flex-col gap-3 md:gap-4 mb-2">
            <div className="flex items-start justify-between gap-3 md:gap-4">
              ...
              <div className="w-full">
                ...
                <p
                  className={`${subTextColor} italic text-xs md:text-sm font-light`}
                >
                  {cabin.krotkiOpisPodNaglowkiem}
                </p>
              </div>
            </div>
            ...
            >
              <div
                className={`text-center p-2 md:p-3 ${
                  isDark ? "bg-[#111]" : "bg-stone-100"
                } rounded-xl ${isDark ? "border-[#222]" : "border-stone-300"} transition-colors group-hover:bg-[#D4A373]/20`}
              >
...
export default function CabinShowcase({ cabins }: CabinShowcaseProps) {
